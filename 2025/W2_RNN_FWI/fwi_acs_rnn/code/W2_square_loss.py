...
def my_grad_func(t,T,f,mu,predictions):
    t = np.array(t)
    T = np.array(T)
    f = np.array(f)
    mu = np.array(mu)
    ...
    for i in range(mu.shape[1]):
       ...
       grad[:,i] = 2*grad[:,i]*predictions[:,i] / int_mu
...
#    b = tf.constant([1, batch_num_source*num_receivers],tf.int32)
 ...
 #   for r in range(batch_num_source*num_receivers):
    ...
    ###gradient w.r.t.f
    def grad(dy):
        residual = tf.numpy_function(func = my_grad_func, inp = [t,T,f,mu,predictions] , Tout=tf.float32)
