...
def gen_data(model, dx, dt, sources, sources_x, receivers_x, propagator,
             batch_size=1):
    ...
    receivers = np.zeros([num_time_steps, num_shots, num_receivers_per_shot],
                         np.float32)
    ...
    for batch_idx in range(num_batches):
        ...
        receivers[:, batch_start : batch_end, :] = \
                sess.run(out_receivers, feed_dict=feed_dict)
