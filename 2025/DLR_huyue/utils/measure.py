...
def compute_measure(x, y, pred, data_range):
    original_psnr = compute_SNR(x, y, data_range)
    original_ssim = compute_SSIM(x, y, data_range)
    original_rmse = compute_RMSE(x, y)
    pred_psnr = compute_SNR(pred, y, data_range)
...
def compute_RRMSE(img1, img2):
    return np.sqrt((((img1 - img2) ** 2).sum())/(((img2) ** 2).sum()))
...
def compute_SNR(img1, img2):
    if type(img1) == torch.Tensor:
        mse_ = compute_MSE(img1, img2)
        signal = compute_MSE( img2,torch.mean(img2))
        return 10 * torch.log10(signal / mse_).item()
    else:
        mse_ = compute_MSE(img1, img2)
        signal = compute_MSE( img2,0)
        return 10 * np.log10(signal / mse_ )
