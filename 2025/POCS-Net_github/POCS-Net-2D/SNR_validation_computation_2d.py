...
parser.add_argument('--end_epoch', type=int, default=100, help='epoch number of end validation')
...
parser.add_argument('--log_dir', type=str, default='SNR_validation2d', help='log directory')
...
filename = 'ValidationDataSet2d.h5'
...
if start_epoch > 0:
    ...
    Loss_epoch = Loss_epoch["Loss_epoch"]
...
with torch.no_grad():
    for epoch_i in range(start_epoch+1, end_epoch+1):
        ...
        for (batch_x,Phi) in rand_loader:
            ...
            x_output = x_output.view(-1,1,64,64)
            for i in range(x.shape[0]):
                original_slice = x[i,:,:,:]
                original_slice = original_slice.view(64,64)
                original_slice = original_slice.cpu().numpy()
                prediction_slice = x_output[i,:,:,:]
                prediction_slice = prediction_slice.view(64,64)
        ...
        sio.savemat("./%s/SNR_validation%d_lr_%.4f_layernum_%02d.mat"%(args.log_dir,epoch_i,learning_rate,layer_num),{"SNR_epoch_validation":SNR_list})
