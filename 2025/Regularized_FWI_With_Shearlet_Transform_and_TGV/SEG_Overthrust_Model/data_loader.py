...
def DataLoad_Train(device):
    ...
    nx = 187
    dx = 25
    v_true = torch.from_file("/home/wanghan/overthrust_model/portion/vp.bin",size=ny*nx).reshape(ny, nx)

    v_true= v_true[100:400, :130].to(device) 
    ...
    dtmax = CourantCondition(dx, 2, 6000.0)
    print("Grid size:", dx)
    ...
    # plt.imshow(observed_data[15].detach().cpu().T, aspect='auto', cmap='Greys',
    #                  vmin=omin/50, vmax=omax/50)
    # plt.savefig('observed_data.png',dpi = 300, bbox_inches='tight',pad_inches=0.01 )
  ...
  #  observed_data /= (observed_data ** 2).mean().sqrt().item()
    ...
    return observed_data, data_dim, model_dim, source_amplitudes, source_locations, receiver_locations, v_true, vmin,vmax,dx,dt,freq,n_shots
