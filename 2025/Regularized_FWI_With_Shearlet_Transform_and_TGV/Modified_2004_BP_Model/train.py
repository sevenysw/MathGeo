...
plt.imshow(seismic_data[15].detach().cpu().T, aspect='auto', cmap='Greys_r',vmin=-0.5, vmax=0.5)
...
v_init = (torch.tensor(1/gaussian_filter(1/v_true.cpu().numpy(), 15)).to(device))
...
#mu matrix
mu = torch.ones(model_dim[0],model_dim[1])
mu_ele1 = torch.linspace(1,300,90)
mu_ele2 = torch.linspace(300,3000,30)
...
lamda_ele1 = torch.linspace(20,1,120)
lamda_ele2 = torch.linspace(1,1,0)
lamda_ele3 = torch.linspace(1,20,120)
...
for row in range(model_dim[1]):
    ...
    lamda_ele= torch.cat([torch.linspace(lamda_ele1[row],1,125), lamda_ele2,torch.linspace(1,lamda_ele3[row],125)])
    lamda[:,row] = lamda_ele
...
# lamda = torch.ones(model_dim[0],model_dim[1])
# lamda_ele1 = torch.linspace(1,30,125)
# lamda_ele2 = torch.linspace(30,1,125)
...
#                                                                                   loss.item()))
    ...
    loss1 = 7*TGV(v,1)+1*shearlet(v,device)
    #loss1 = 2*shearlet(v,device)
    loss1.backward()
    #grad_tgv = (v.grad).detach().cpu().T - grad_l
    optimizer.step()
    v.data = torch.clamp(v.data, min=1300, max=4700)
